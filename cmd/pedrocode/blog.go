...
func runBlogMode(debugMode bool) error {
	// Load config
	cfg, err := loadConfig()
	if err != nil {
	...
	// Check if blog mode is enabled
	if !cfg.Blog.Enabled {
		return fmt.Errorf("blog mode is not enabled in config")
	}
	...
	// Create session
	sessionID := fmt.Sprintf("blog-%s-%s", uuid.New().String()[:8], time.Now().Format("20060102-150405"))
	session, err := repl.NewSession(sessionID, cfg, bridge, "blog", debugMode)
	if err != nil {
