...
func (t *DictationTool) Execute(ctx context.Context, args map[string]interface{}) (*tools.Result, error) {
	...
	if !ok {
		return tools.ErrorResult("missing 'transcription' argument"), nil
	}
	...
	if err != nil {
		return tools.ErrorResult(fmt.Sprintf("failed to create draft: %v", err)), nil
	}

	return &tools.Result{
		Success: true,
		Output:  fmt.Sprintf("Created blog post draft: %s (ID: %s)", post.Title, post.ID),
...
func (t *ProcessWriterTool) Execute(ctx context.Context, args map[string]interface{}) (*tools.Result, error) {
	...
	if !ok {
		return tools.ErrorResult("missing 'post_id' argument"), nil
	}

	postID, err := uuid.Parse(postIDStr)
	if err != nil {
		return tools.ErrorResult(fmt.Sprintf("invalid post_id: %v", err)), nil
	}

	job, err := t.manager.ProcessWithWriter(ctx, postID)
	if err != nil {
		return tools.ErrorResult(fmt.Sprintf("failed to process with writer: %v", err)), nil
	}

	return &tools.Result{
		Success: true,
		Output:  fmt.Sprintf("Writer agent job started: %s", job.ID),
		Data: map[string]interface{}{
			"job_id":  job.ID,
			"post_id": postIDStr,
...
func (t *ProcessEditorTool) Execute(ctx context.Context, args map[string]interface{}) (*tools.Result, error) {
	...
	if err != nil {
		return tools.ErrorResult(fmt.Sprintf("failed to process with editor: %v", err)), nil
	}

	return &tools.Result{
		Success: true,
		Output:  fmt.Sprintf("Editor agent job started: %s", job.ID),
		Data: map[string]interface{}{
			"job_id":      job.ID,
			"post_id":     postIDStr,
			"status":      string(job.Status),
...
func (t *PublishTool) Execute(ctx context.Context, args map[string]interface{}) (*tools.Result, error) {
	...
	if addNewsletter, ok := args["add_newsletter"].(bool); ok && addNewsletter {
		if err := t.manager.AddNewsletterSection(ctx, postID); err != nil {
			return tools.ErrorResult(fmt.Sprintf("failed to add newsletter section: %v", err)), nil
		}
	}

	// Publish to Notion
	if err := t.manager.PublishToNotion(ctx, postID); err != nil {
		return tools.ErrorResult(fmt.Sprintf("failed to publish to Notion: %v", err)), nil
	}
	...
	if substackURL, ok := args["substack_url"].(string); ok && substackURL != "" {
		if err := t.manager.MarkAsPublished(ctx, postID, substackURL); err != nil {
			return tools.ErrorResult(fmt.Sprintf("failed to mark as published: %v", err)), nil
		}
	}
...
func (t *ListPostsTool) Execute(ctx context.Context, args map[string]interface{}) (*tools.Result, error) {
	...
	if err != nil {
		return tools.ErrorResult(fmt.Sprintf("failed to list posts: %v", err)), nil
	}
